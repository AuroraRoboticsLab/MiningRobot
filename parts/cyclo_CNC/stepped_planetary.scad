...
v5: leave space between eccentric gear teeth for mounting bolts
    upgrade outside bolts to #10 thru
    shrink gear area to make space around inside mount bolts

v6: Full semicircle teeth, using 3/16" (0.1875") rods.
...
mainboltODO = 0.192*inch+0.2; // #10 machine screw thru   5.0; // shaft diameter + clearance, M5 bolt
mainboltTapO = 4.5; // tap diameter
mainboltShaftZO = 30; // length of clear shaft portion
mainheadODO=9.0;
mainheadZO=5.0;
...
extendEZ=0.5; // extend eccentric gear teeth this far in Z (leaves space for bolts)
...
module mainboltI(shaft=mainboltShaftZI) { // smooth, for tap: mainboltShaftZI) {
...
module version2D(versionText="") {
    text(str(versionText,"v6"),size=3,halign="center",valign="center");
}

/* Eccentric gears mate to both ring gears, and transfer power */
module eccentric_gears() {
    bottomZ = gBZ+axleringZ;
    bottomDZ = gearZ + extendEZ - axleringZ - 0.8;
    topZ = gTZ-axleringZ;
    topDZ = extendEZ - axleringZ; // extent of top gear half
    difference() {
        union() {
            ...
            translate([0,0,bottomZ]) {
                 toothspot_bottom() {
                    ...
                    rotate_extrude() translate([solidOD/2-0.1,donutZ+1]) 
                        scale([0.5,1]) circle(r=3.0);
                    */
                    solidODB=solidOD+1.1; // bottom taper
                    solidODT=solidOD+3.9; // top taper
                    round=1.8;
                    // Smooth transition between gear inner faces
                    rotate_extrude() translate([0,donutZ]) 
                    offset(r=-round) offset(r=+2*round) offset(r=-round)
                    {
                        translate([solidODB/2,0]) scale([-1,-1]) square([5,10]);
                        translate([solidODT/2,0]) scale([-1,+1]) square([5,10]);
                    } 
                }
            }
...
if (0) { /* don't print anything (include) */ }
    ...
    demo3D(); // assembled 3D view, with cutaway
}
