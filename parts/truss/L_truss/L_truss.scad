...
ECHO: "Bay diag: ", 911.616 (assembles easier at 900mm, for some slack)
ECHO: "Top diag: ", 911.616
Full set of 0-8 vertexes.

Half length single-bay 5ft / 1.5m version:
ECHO: "Top long rods: ", 1494
ECHO: "Bot long rods: ", 1303.9
Others: same as full size.
The only vertexes needed are 0-3 and 5.
...
// Retaining screws in YZ face of endplate, for screwing down into wood.
...
// Print a CAM template for fabricating a steel endplate top
module endplate_template()
{
    difference() {
        // Main plate
        translate([0,-endplateY/2])
            square([truss_top_plateX,endplateY]);
        
        for (dY=[-1,0,+1]) translate([truss_top_screwX,truss_top_screwsR*dY])
            circle(d=3);
        ...
        endplate_shift(flipX,flipX!=flipZ,flipZ) {
            ...
            translate([truss_top_screwX,0,0])
            {
                ...
                difference() {
                    union() {
                        cylinder(d=tOD,h=tZ);
                        // Bevel to avoid stress riser where we meet plate
                        bevel=4;
                        cylinder(d1=tOD+2*bevel,d2=tOD,h=bevel);
                    }
                    // Cut tapered screw hole
                    translate([0,0,truss_wall])
                    cylinder(d1=truss_top_screwOD,d2=tOD,h=tZ+0.1);
                    
                    // Trim top of bevel above end of plate 
                    translate([-truss_top_screwX+truss_top_plateX+25-1,0,0]) cube([50,50,50],center=true);
                }
            }
        }
}
...
module tube_between(p1,p2, ODraw, enlarge, retain, lengthen)
{
    ...
    translate([0,0,-lengthen-enlarge]) {
        ...
        cylinder(d=OD,h=len);
        if (graphics_mode==0 && ODraw<rodOD_big) {
...
module truss_tubes(enlarge=-0.01,outofplane=0,edgestyle=1,longonly=0,longextra=0)
{
    ...
    if (!longonly) {
        ...
        if (edgestyle==3) { // end diagonals (on either end)
            ...
            tube_list([flip(TL),flip(shift_bay(BL,+1))],d,enlarge,retain_diag,lengthen_diag); 
            tube_list([flip(BL),flip(shift_bay(TL,-1))],d,enlarge,retain_diag,lengthen_diag); 
        }
    }
}
...
edgestyle=3;
truss_vertex_printable(3,edgestyle=edgestyle);
